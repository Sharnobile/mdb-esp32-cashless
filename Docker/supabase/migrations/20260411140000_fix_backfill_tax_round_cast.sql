-- =========================================================
-- Fix: backfill_sales_tax() — explicit ::numeric casts for ROUND
--
-- Same class of bug as the stamp_machine_and_decrement_stock trigger
-- fixed in 20260411120000_fix_tax_trigger_round_cast.sql:
--
-- The original 20260406200000_tax_backfill_function.sql (commit aabc5d6)
-- was committed with ROUND(s.item_price / ..., 4) where item_price is
-- float8, producing "function round(double precision, integer) does not
-- exist" at runtime. The file was later edited in commit 69effe6 to add
-- ::numeric casts, but update.sh filename-based tracking means existing
-- DBs keep the original buggy version.
--
-- This function is an on-demand RPC (called from the Reports page to
-- stamp historical sales with tax data), not a trigger — so the bug
-- only manifests when an admin actually calls it, not on every sale.
--
-- Idempotent: CREATE OR REPLACE. No-op on DBs that somehow already
