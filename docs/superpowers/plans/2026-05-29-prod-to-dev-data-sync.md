...
- **Supabase CLI dependencies verified (2026-05-30):** `supabase status -o env` and `supabase migration up` run cleanly on the dev machine, and the `SERVICE_ROLE_KEY` extraction (`sed -n 's/^SERVICE_ROLE_KEY="\(.*\)"$/\1/p'`) returns a valid 164-char key. (An earlier review flagged a `ProjectEnvParseError` from a multi-line `APNS_PRIVATE_KEY`, but that was a review-sandbox artifact — not reproducible on the real machine, so no `.env` change is needed.)
