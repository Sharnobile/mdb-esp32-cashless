...
func chipName(_ chip: ChipFilter) -> String {
    switch chip {
    case .all:
        return String(localized: "refill.chip.all")
    case .machine(let id):
...
/// Mirrors `packAllMachines` but scoped — drives the "%@ komplett packen"
...
Search the file for `func loadData` (likely around line 800–900). Locate the function and add this line at the very end of the success path (right before the closing `}` of the `do` block, after `machines = ...` is assigned):

```swift
activeChip = .all
```

If `loadData()` has multiple success branches (re-load vs. first-load), apply the reset to both, OR place it once at the function's final exit point after all assignments have settled. Use `grep` to confirm there is only one assignment to `machines`:

```bash
grep -n "machines = " /Users/lucienkerl/Development/mdb-esp32-cashless/ios/VMflow/ViewModels/RefillWizardViewModel.swift
```
...
**Rationale:** `Localizable.xcstrings` is JSON. New keys go into the top-level `"strings"` object. Each key has `"extractionState": "manual"` and `"localizations"` with `en` and `de`. Pattern-match against an existing entry (e.g. search for an existing `"refill."` prefix key) to copy the exact JSON shape.

- [ ] **Step 1: Open the xcstrings file**

Open `ios/VMflow/Resources/Localizable.xcstrings` in Xcode (preferred — gives the table editor) or your text editor.

- [ ] **Step 2: Add the eight keys**

Add the following eight entries to the top-level `"strings"` JSON object. If Xcode's table editor is used: click "+" to add each key, set English value, switch to German and set the German value, and ensure state is "manual" (translated). If editing the JSON directly, append the entries shown below — exact key spelling matters:

| Key | EN | DE |
| --- | --- | --- |
| `refill.chip.all` | `All` | `Alle` |
| `refill.header.all.pending` | `All machines · %lld items · %lld/%lld ready` | `Alle Automaten · %lld Items · %lld/%lld ready` |
| `refill.header.all.done` | `✓ All boxes packed · %lld items` | `✓ Alle Boxen komplett · %lld Items gepackt` |
| `refill.header.machine.pending` | `%@ · Box: %lld items · %lld/%lld packed` | `%@ · Box: %lld Items · %lld/%lld gepackt` |
| `refill.header.machine.done` | `✓ %@ · Box complete · %lld/%lld packed` | `✓ %@ · Box komplett · %lld/%lld gepackt` |
| `refill.header.machine.alert` | `⚠ %@ · new sale · box now %lld items (+%lld)` | `⚠ %@ · neuer Verkauf · Box jetzt %lld Items (+%lld)` |
| `refill.selectAll.machine` | `Pack all for %@` | `%@ komplett packen` |
| `refill.chip.itemCountSeparator` | ` · ` | ` · ` |

**Format-specifier note:** Use `%lld` for `Int` arguments in Swift's `String(localized:)` macro — that's what Foundation's bridging picks. `%@` for `String` arguments. The order of the `%`-placeholders must match the order in which you'll pass arguments at the call site.

- [ ] **Step 3: Build to verify the JSON is well-formed**

In Xcode: ⌘B. Expected: no errors. If Xcode shows "string catalog corrupted" or similar, re-open the file in a text editor and check for a misplaced comma or unmatched brace.
...
private struct HeaderStrip: View {
    ...
    private var headerText: String {
        ...
        case .all:
            ...
            if state == .done {
                return String(localized: "refill.header.all.done \(total)")
            } else {
                let ready = viewModel.packedMachines.count
                let totalM = viewModel.machines.count
                return String(localized: "refill.header.all.pending \(total) \(ready) \(totalM)")
            }
        case .machine(let id):
            ...
            switch state {
            case .done:    return String(localized: "refill.header.machine.done \(name) \(packed) \(needs.count)")
            case .alert:
                ...
                return String(localized: "refill.header.machine.alert \(name) \(total) \(extra)")
            case .pending: return String(localized: "refill.header.machine.pending \(name) \(total) \(packed) \(needs.count)")
            }
        }
    }
    ...
    private var selectAllLabel: String {
        ...
        case .machine(let id):
            let name = viewModel.chipName(.machine(id))
            return String(localized: "refill.selectAll.machine \(name)")
        }
    }
}
```
...
private struct MachinePackingList: View {
    ...
    private var emptyState: some View {
        VStack(spacing: 12) {
            ...
            Text("No products to pack for this machine")
                .font(.subheadline)
...
`summaryHeader` itself stays on the struct as dead code for one commit (so the diff stays small), then gets deleted in the polish task below.
...
- Line counts roughly within the spec's estimate (~50-65 VM, ~150-200 View, ~8 xcstrings entries).
