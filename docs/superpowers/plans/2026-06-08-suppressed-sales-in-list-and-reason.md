...
git commit -m "feat(pwa): show auto-removed sales in the Sales list (marked, restorable) + richer reason

Interleave suppressed sales into the day-grouped Sales feed as dimmed,
non-counting rows (Auto-removed badge + strikethrough price + reason) with
an admin-only 'Take up as sale' button (reuses the existing restore flow);
...
- [ ] **Step 2: Convert `salesTab` to a `List` with swipe-to-restore** — replace the ENTIRE `salesTab` computed property (`private var salesTab: some View { ScrollView { … } .refreshable { … } }`, ≈ lines 313–346) with a `List(.plain)` version mirroring the Duplicates-tab pattern. Marked suppressed rows get an admin-only swipe action; real sale rows keep their product-sheet tap:
```swift
...
```
This preserves the empty-state ("No sales yet"), the `.refreshable` (now on the `Group`), and the day-grouped sections; it swaps `ScrollView { LazyVStack }` → `Group { … List(.plain) }` and adds the admin-only swipe on suppressed rows. `rowToRestore`/`showRestoreConfirm`/`isAdmin` already exist on `MachineDetailView` (prior milestone). (`SaleRow`'s `.onTapGesture` for the product sheet still fires inside a `List` row.)

- [ ] **Step 2b: Hoist the shared `.confirmationDialog` to the `TabView`** — the restore confirm dialog currently hangs off `suppressedTab` (prior milestone). Now BOTH tabs trigger restore, so move it to the common `TabView`.

(a) In `suppressedTab`, DELETE the trailing block (after `.refreshable { await viewModel.loadDetail() }`):
```swift
        .confirmationDialog(
...
```
so `suppressedTab` now ends at its `.refreshable { await viewModel.loadDetail() }`. (Its swipe still sets `rowToRestore` + `showRestoreConfirm`.)

(b) In the view `body`, attach that SAME dialog to the `TabView`. Change:
```swift
...
- [ ] **Step 5: Build in Xcode** — build the `VMflow` scheme; expect BUILD SUCCEEDED. Manually verify: the Sales tab (now a `List`) interleaves dimmed, strikethrough "Auto-removed" rows (with the clock+gap reason) by day; the day-header count counts real sales only; revenue unchanged; **swiping a marked Sales-tab row reveals an admin-only green "Take up as sale"** → the (shared) confirm dialog → restoring makes it leave the marked set and appear as a real sale with stock −1; the Duplicates-tab swipe still works (same shared dialog) and shows the richer reason. (If Xcode unavailable, report that a manual build is required.)
...
git commit -m "feat(ios): auto-removed sales in the Sales tab (marked, swipe-to-restore) + richer reason

Convert the Sales tab to a List; interleave suppressed sales as dimmed,
strikethrough 'Auto-removed' rows with a clock+gap reason + an admin-only
swipe 'Take up as sale'; hoist the shared restore confirm dialog to the
TabView. Counts/revenue unchanged; Duplicates tab reason shows the same
circumstances.
...
## Done criteria
- PWA Sales tab shows auto-removed sales interleaved by day as dimmed, non-counting rows (orange "Auto-removed" badge + strikethrough price + clock/gap reason) with an **admin-only "Take up as sale" button**; revenue + 30-day chart unchanged; `useSuppressedSales` pure helpers unit-tested; vitest suite green; build clean.
- iOS Sales tab (now a `List`) shows the same marked rows (unified `SalesFeedItem` feed, day-grouped, real-only count) with an **admin-only swipe "Take up as sale"**; the restore confirm dialog is **hoisted to the `TabView`** and shared by both tabs; Duplicates tab unchanged except the richer `reasonText` + the moved dialog; builds in Xcode.
- Restoring a marked row (PWA button / iOS swipe) promotes it to a real sale (stock −1) via the existing `restore_suppressed_sale` flow, removes it from the auto-removed set, and shows it as a normal sale.
