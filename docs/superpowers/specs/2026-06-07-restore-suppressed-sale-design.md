...
- **Admin gating:** show the swipe action only when the user is an admin (`AuthService.shared.role == .admin`). NOTE this is the **first sale-mutation admin gate in the iOS app** — there is no existing idiom to copy. The role is available via `AuthService.shared.role` (an `OrganizationRole?`); `SettingsView` is currently the only reader, and only to *display* it. The RPC enforces admin server-side regardless, so a non-admin who somehow triggers it gets a surfaced error rather than a silent change.
